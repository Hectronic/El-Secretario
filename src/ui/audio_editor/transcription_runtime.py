"""Runtime boundary for retranscribing an edited recording."""
...
class RetranscriptionStartResult:
    started: bool
    preflight_error: str | None = None


class AudioEditorTranscriptionRuntime:
    """Create and safely clean up the one retranscription worker for an editor."""
    ...
    def start(self, audio_path, duration, on_finished, on_error, on_started=None):
        if self.thread and self.thread.isRunning():
            return RetranscriptionStartResult(started=False)
        ...
        if preflight_error:
            return RetranscriptionStartResult(started=False, preflight_error=preflight_error)

        language = settings.value("rec_config/language", "Auto")
        language_code = {"Auto": None, "Spanish": "es", "English": "en"}.get(language)
        ...
        if on_started:
            on_started()
        self.thread.start()
        return RetranscriptionStartResult(started=True)
