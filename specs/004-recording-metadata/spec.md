...
- Given a recording widget receives an injected persistence port, when it loads a
  recording or opens a chat context, then it uses that port rather than creating a
  separate database connection.

## Architecture Notes

- UI: `src/ui/recording_widget.py` owns the recording-tab Qt façade. `src/ui/recording/layout.py` owns normal and audio-edit visual composition; `transcription_panel.py`, `metadata_panel.py`, `content_tabs.py`, and `actions_bar.py` own focused panel construction. `src/ui/recording/controls.py` owns shared recording-tab button/playback-control factories so action styling, playback controls, and enabled-state defaults stay consistent.
...
- Persistence: recording and note workflows accept injected aggregate persistence;
  `src/database.py` remains the compatible default facade while
...
- Other docs: `specs/README.md` links this feature and records `src/ui/recording/` as the focused recording UI package.
