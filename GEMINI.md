...
6. **Pull Requests and Git Workflow (AI Autonomous PR creation)**:
...
Use the GitHub Spec Kit workflow whenever a task modifies code that is not yet well refactored, is listed as a hotspot, or lacks a matching feature artifact in `specs/`.

Activation checks:
- The changed behavior is not covered by an existing `specs/<NNN-feature>/spec.md`.
...
- Update the relevant `spec.md`, `plan.md`, and `tasks.md` in `specs/<NNN-feature>/`, or create a new numbered feature directory when the behavior has no product contract.
- Record ownership, follow-up work, and completion state in that feature's Spec Kit artifacts.
...
Expected output:
- A short note naming the Spec Kit feature artifact updated.
