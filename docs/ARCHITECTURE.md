...
- `src/ui/recording_widget.py` is now mostly a Qt orchestration shell for recording detail and legacy audio-edit tabs. Focused recording-tab UI builders, controls, state helpers, detail loading/persistence, direct transcription flow helpers, AI/recording actions, speaker mapping, audio trim helpers, and RAG indexing helpers live under `src/ui/recording/`; remaining risk is broad transcription/AI and dirty-state orchestration.
...
2. Continue shrinking `RecordingWidget` by moving transcription/AI completion orchestration and dirty-state coordination into focused modules; detail loading/persistence, deletion, open-chat, playback adapters, UI builders, shared controls, direct transcription flow helpers, AI actions, speaker mapping, audio trim helpers, and RAG indexing helpers already live under `src/ui/recording/`.
