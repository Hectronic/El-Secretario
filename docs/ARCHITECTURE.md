...
- `src/ui/recording_widget.py` is now mostly a Qt orchestration shell for recording detail and legacy audio-edit tabs. Focused recording-tab UI builders, controls, state helpers, detail loading/persistence, direct-transcription and AI orchestration, speaker mapping, audio trim helpers, and RAG indexing helpers live under `src/ui/recording/`; remaining risk is dirty-state and legacy audio-edit orchestration.
