...
- `src/ui/recording_widget.py` is now mostly a Qt orchestration shell for recording detail and legacy audio-edit tabs. Focused recording-tab UI builders, controls, small state helpers, direct transcription flow helpers, AI/recording actions, speaker mapping, audio trim helpers, and RAG indexing helpers live under `src/ui/recording/`; remaining risk is record loading/saving orchestration and persistence coupling.
...
2. Continue shrinking `RecordingWidget` by moving record loading/saving and persistence-facing orchestration into focused modules/services; deletion, open-chat, playback adapters, UI builders, shared controls, direct transcription flow helpers, AI actions, speaker mapping, audio trim helpers, and RAG indexing helpers already live under `src/ui/recording/`.
