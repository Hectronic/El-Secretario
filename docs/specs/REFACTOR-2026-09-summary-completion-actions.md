# REFACTOR-2026-09-SUMMARY-COMPLETION: Queue Policy And State
...
- Preserve completion follow-ups, task admission, deduplication, pending ordering,
  active-task transitions, and retry-wait behavior for queued work.
...
- To: `src/app/summary_queue/completion_actions.py`,
  `src/app/summary_queue/state.py`, `src/app/summary_queue/wait_state.py`, and
  `src/app/summary_queue/admission.py`.

## Tests

- Focused: `tests/app/summary_queue/test_completion_actions.py`,
  `tests/app/summary_queue/test_admission.py`,
  `tests/app/summary_queue/test_state.py`,
  `tests/app/summary_queue/test_wait_state.py`,
